...
> **Frozen design record — point-in-time snapshot, 2026-08-10.**
>
> This is the plan as approved before implementation of #208 began. It is
> deliberately **not** updated as work proceeds: issues #208–#211 and the code
> itself are the live record, and `zephyr/sophon/PROTOCOL.md` holds the live
> wire-format contract.
>
> It is kept because the reasoning here is not recoverable from the source once
> written — the ATT MTU budget, the connection-event supply/demand derivation,
> the batching-vs-DLE arithmetic, and why the Zephyr app is freestanding. Those
> explain *why* the frame is 18 bytes, why the sample rate is 50 Hz, and why the
> MTU is left at its default.
>
> It carries analysis for **both** halves despite living under `ios/`.
...
```
Sophon Motion Service   <B[0]=…0001>-…    primary service, in the adv payload
Motion Data (notify)    <B[0]=…0002>-…    18-byte frame above, + CCC
```
...
# Freestanding app: the Zephyr revision comes from whatever ~/zephyrproject is at,
# not from this repo. Fail at configure time with a readable message rather than
# on some unrelated compile error three layers down.
if(CONFIG_ZEPHYR_VERSION_CODE LESS 0x040400)   # 4.4.0
  message(FATAL_ERROR
    "Sophon requires Zephyr >= 4.4.0; found ${ZEPHYR_VERSION} at ${ZEPHYR_BASE}. "
...
The exact guard mechanism gets confirmed against Zephyr 4.4's actual CMake
variables during implementation (`ZEPHYR_VERSION_CODE` vs `ZEPHYR_VERSION` vs
`Zephyr_VERSION`) — the intent is fixed, the spelling is verified before it ships.
...
# USB CDC ACM console — the XIAO's only console path
CONFIG_USB_DEVICE_STACK=y
CONFIG_USB_CDC_ACM=y
CONFIG_USB_DEVICE_INITIALIZE_AT_BOOT=y     # verified: depends on USB_CDC_ACM
CONFIG_USB_DEVICE_PRODUCT="Sophon"
CONFIG_BOOT_DELAY=5000                     # let CDC enumerate before logs print

CONFIG_LOG=y
CONFIG_GPIO=y
```

`CONFIG_BOOT_DELAY=5000` is the board doc's own recommendation and is the
difference between seeing the boot banner and seeing nothing.
...
```
ios/Sophon/
├── ORIGINAL-PLAN.md               ← this document, frozen (see below)
├── Sophon.xcodeproj/
...
### `ios/Sophon/ORIGINAL-PLAN.md` — the frozen design record

**First file written during implementation**: this plan document, copied verbatim
into `ios/Sophon/ORIGINAL-PLAN.md` with a short header stating it is a
point-in-time snapshot (2026-08-09), superseded by the issues and the code as work
progresses, and deliberately **not** updated afterwards.

The value is in the parts that will otherwise be lost: the ATT MTU budget, the
connection-event supply/demand derivation, the batching-vs-DLE arithmetic, and the
reasoning behind the freestanding-app choice. Those explain *why* the frame is 18
bytes, why the sample rate is 50 Hz, and why the MTU is left at its default — none
of which is recoverable from the source once written.

Two consequences for the rest of the plan:

- `zephyr/sophon/PROTOCOL.md` **links to it rather than duplicating it**, and keeps
  only the live contract (frame layout, frozen UUIDs, known gaps). One document
  drifts, the other is frozen; duplicating the analysis across both guarantees they
  disagree.
- It carries analysis for both halves despite living under `ios/`, so
  `zephyr/CLAUDE.md` and `zephyr/sophon/README.md` point at it explicitly — nobody
  working on the firmware should have to guess that the BLE capacity derivation is
  filed under the iOS target.
...
0. `ios/Sophon/ORIGINAL-PLAN.md` exists and matches this document, and the two
   pointers to it (`zephyr/CLAUDE.md`, `zephyr/sophon/README.md`) resolve.
