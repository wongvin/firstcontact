...
- Every image so far arrived the same way: build, convert to UF2, double-tap reset, copy to a mounted volume. That works only for application images placed above the Adafruit bootloader, and the bootloader is itself in flash — overwrite it and there is no way back. #253 establishes an SWD path that removes both limits, and proves the round trip before relying on it.
- **The deliverable was never "MCUboot boots".** It was a demonstrated write-and-recover loop, and that ran twice: a full-flash rehearsal, then `nrf52_recover` (mass erase + unlock) leaving the chip blank — 0 non-FF bytes in 1 MB of flash *and* 4 KB of UICR — followed by a restore that came back byte-identical, booted, and mounted `XIAO-SENSE`. UICR being writable was the genuine unknown; had it not been, the discovery would have come during a real lockout with nothing to restore from.
- **SWD test points established from the KiCad netlist, not by probing pads.** TP1 GND, TP2 RESET, TP3 SWDCLK, TP5 SWDIO — a 2×2 grid on the back at 2.54 mm pitch, positions recorded relative to the board edges. No TP2 connection is needed: `sysresetreq` covers reset and CTRL-AP (`IDR 0x02880000`) covers mass erase and unlock, so the probe's missing reset line costs nothing.
- **`app.overlay` could not supply the MCUboot layout**, because the board defines no partitions at all — its layout comes from `nordic/nrf52840_partition_uf2_sdv7.dtsi`, pulled in by `xiao_ble_common.dtsi` and therefore invisible to any grep of the board directory. Two overlays in `swd/`: the partition table, and a separate `code-partition` choice for the application, which cannot live in the shared file because MCUboot's own `app.overlay` points itself at `boot_partition` and application overlays are applied after it.
- **The layout wasted 120 KB on the first attempt.** Zephyr's `nrf52840dk` table was copied wholesale; it is written for swap-using-scratch, while sysbuild selects `CONFIG_BOOT_SWAP_USING_OFFSET=y`, which swaps in place and needs no scratch. Removing it took the slots from 420 KB to **480 KB** each.
- **`sysbuild/mcuboot.conf`** exists because MCUboot inherits the board's devicetree while being a minimal single-threaded build. Two nodes do not survive that, and both fail at *link* time minutes into a build: the CDC ACM console (`undefined reference to __device_dts_ord_127`) and `regulator-fixed`, which calls `k_usleep` under `CONFIG_MULTITHREADING=n`.
- **MCUboot now has a console on `uart0`.** The first fix for that link error was to switch its console off entirely, which built and cost more than it saved — the bootloader logged nothing, so its stage of the boot was invisible, and answering "how long does MCUboot take" fell back to sampling the program counter over SWD. That halts the CPU, perturbs the measurement, and strands the BLE link. A plain UARTE needs no USB stack. **992 bytes**, and MCUboot now narrates its own boot: chainload offset, image version, and the trailer state that explains why nothing has ever reverted. Timestamps are available for a further 3.8 KB and declined — 89.5% of the boot partition is too little headroom for the one image that recovers all the others.
- **The most expensive finding: never end an SWD session with `resume`; always `reset run`.** Halting stops Zephyr's link-layer controller and its timers together, so no supervision timeout can fire locally — the peer gives up at 420 ms while the board notices nothing. On resume the board still believes the dead connection is live: it stops advertising, refuses new centrals, and only a reset recovers it. Demonstrated deliberately with a 30-second halt, no flashing and no image change: unreachable for over a minute, then connected 115 ms after a reset. This is why the symptom correlated with MCUboot without MCUboot being at fault — SWD is what brought halting into the workflow.
- Related OpenOCD hazards, same root cause: **any operation that runs code on the target needs a sane core.** `verify_image` loads a CRC routine into RAM, times out on 1 MB and leaves a HardFault — use a host-side `cmp` against a fresh dump. Erase and write must be **separate sessions** with `reset halt` between them, or the write fails with `timeout waiting for algorithm` because the core is locked up executing blank flash.
- **`build.sh` takes `SOPHON_BOOT=uf2|mcuboot`**, defaulting to `uf2` for the transition — the boards have diverged and `Sophon-4D88` is not migrated. Separate build directories per mode. The shared prologue moved to `scripts/common.sh`, which was already duplicated between `build.sh` and `flash.sh` before this change.
- **New `scripts/flash-swd.sh`**, which refuses to write to a board that still has a SoftDevice at `0x3004` — flashing MCUboot over a UF2 board destroys its bootloader *and* its S140, and there is no backup of the un-migrated board. `flash.sh` gains staleness guards and names the other script; the silent failure it prevents is building MCUboot and then flashing a stale UF2 by habit.
- **New `zephyr/sophon/BOOTLOADER.md`**, 519 lines: the two boot paths, three-column memory maps, what a SoftDevice is and that S140 v7.3.0 really was present, SWD access and recovery, the MCUboot configuration, all three serial paths drawn end to end, the hazards, the transition, and a "Not established" section for the four things that remain open. Wrong turns are kept rather than tidied away — each produced a plausible result rather than an error, which is the only reason they cost anything.
- `.gitignore` gains `zephyr/*/build-*/`. The existing rule matches only a directory named exactly `build`, so sysbuild's tree — 1,916 files — was untracked and burying the three that mattered.

## 2026-09-08

### feat: report battery voltage (#268)

- A 200 mAh LiPo is now connected to `Sophon-86F0` and the board said nothing about it — it streamed until it stopped, and a peripheral that died mid-session was indistinguishable from one carried out of range.
- **Voltage and nothing else.** There is no fuel gauge on a XIAO nRF52840: the BQ25101 is a charger, so nothing counts charge in or out, and a LiPo's discharge curve is nearly flat across 3.7–3.9 V — most of its usable range. Under a 52 Hz IMU and an active radio the terminal voltage also sags in bursts, biasing any estimate low exactly when a connected Sophon would sample it. A percentage or an mAh figure would be modelled rather than measured, which is the failure #228, #230, #237 and #263 each exist to correct.
- New `voltage-divider` node and SAADC channel in `app.overlay`, because the board files declare neither — no divider, no named enable pin, and `&adc` enabled with no channel children. Zephyr's driver owns the scaling; `battery.c` averages **eight conversions spaced 10 ms apart**, spread rather than bursted so the mean straddles both the radio's transmit bursts and the quiet between them.
- **The age of the reading is on the wire.** The board samples on its own 60 s timer and the GATT read returns that cache, so a read proves a central asked and nothing more. A consumer's true age is the sum of the board's figure and the time since the read — neither alone is the answer. This is #237's crediting rule applied before it could go wrong again.
- **`R17` is 499 kΩ, not 510 kΩ** — found only after switching from the PDF to the KiCad source. Seeed's wiki archive is *named* `…v1.1.zip` but contains the **v1.0** schematic where R17 is 510k; the v1.1 `.kicad_sch` says 499k. A 1.46% error in the divider ratio, ~60 mV at a full pack, which reads as a perfectly ordinary battery voltage. **Which revision this board is remains unconfirmed** — `TEST-PLAN.md` §5b has the meter check that settles it.
- **P0.14 is held low permanently, and must never be released.** Seeed's FAQ warns that P0.14 high can push P0.31 to its 3.6 V limit; the arithmetic puts it at exactly 3.60 V at a full pack, since P0.14 is the divider's low leg. High-impedance is worse — nothing holds the node down and R16 pulls it to the pack voltage. An interim build released the pin between samples, via `power-gpios` and `CONFIG_PM_DEVICE`, and both are now deliberately absent. The cost of holding it low is ~2.8 µA, about 25 mAh a year against a pack whose own self-discharge is several times that.
- Dropping `CONFIG_PM_DEVICE` also removed the trip hazard that **killed the IMU**: enabling device runtime PM alongside the divider changed every device's lifecycle and stopped the LSM6DS3TR-C data-ready trigger firing, while init still reported success.
- **The app says only what the hardware supports.** VBAT is the charger's `OUT`, the `BAT` pad and the divider shorted together, so no measurement there can say what is driving it. The firmware captures **VBUS with each sample**, and the row is labelled `Battery` when USB is absent — the board can only be running off its pack — and **`VBAT net`** when USB is present. Pack *absence* stays undetectable and is never claimed: `/CHG` would have been the extra evidence, but P0.17 lands on the BQ25101's `PRETERM` input, not its charge status.
- New **`zephyr/sophon/scripts/kicad-netlist.py`** — resolves nets and component values from a `.kicad_sch`, which is what settled the questions the PDF got wrong. Committed as the *generator*, not its output: the script is ours, while a netlist is a derivative of a CC BY-SA 4.0 design and this repo is public with no LICENSE. It also makes every hardware claim reproducible rather than asking to be taken on faith. Its docstring states what it does not handle — hierarchical sheets, buses, no-connects — because on a multi-sheet design it would silently under-report rather than fail.
- New **`zephyr/sophon/HARDWARE.md`** — everything established about this board's battery sensing in one place: the netlist-derived divider, R16/R17 with the unresolved revision question, why P0.14 must be held low, the charger and the pins that are not what their names suggest, the ADC choices and accuracy budget, the confirmed traps, and how to read the schematic from either the KiCad source or the PDF. Separate from `PROTOCOL.md` (the wire format) and `README.md` (build and flash) because it describes the **board** and would hold for any firmware on it. `PROTOCOL.md` and `README.md` now point at it rather than restating it — two copies of this would drift, which is #260.
- `zephyr/sophon/README.md` gains a **The board** section with the KiCad and PDF sources, and why the source is authoritative: #268 mis-identified P0.17 twice from the PDF, once from the net name and once from where symbols sat on the page.
- **Four defects caught by `swift-reviewer` before this landed**, two of them lessons from issues shipped the same week. The battery row asserted a current voltage for a released link — the #263 failure, with State saying `Released` while this row showed an unqualified number. `battery == nil` conflated *read in flight* with *does not report*, so a healthy board displayed a footer naming three wrong causes until its first read returned — also #263, reintroduced within a day. `batteryCharacteristics` was cleared in neither `forget()` nor the disconnect path, against an invariant `forget()`'s own comment states. And the `usbPowered == nil` case fell through to `Battery`, making the strongest available claim exactly where nothing is known; it now takes the weaker `VBAT net`, which is true in every case.
- Two smaller ones: the equality guard in `ingestBattery` compared whole readings including the board's free-running cache age, so it fired on nearly every poll while reading as protection; and the new poll's doc comment had been inserted *inside* `pollRSSI`'s, leaving one merged block claiming both "buys no air traffic" and "genuinely is an over-the-air round trip" three paragraphs apart. That is #260's defect class, in a diff written days after fixing it.
- `batteryStaleAfter` raised 180 s → 240 s. The firmware latches the last good reading on a failed conversion without refreshing its timestamp, so one failure puts the total age at exactly 180 s — the margin the old rationale claimed did not exist.
- `TEST-PLAN.md` gains **§5, 19 cases**, including 5.2 — a trap rather than a case, since running the calibration check with the pack removed lets the meter and the divider between them sag a node nothing is holding up.
...
- The **Interval**, **Peripheral latency** and **Supervision timeout** rows vanished for a second or two on reconnect and then came back. `resetLinkStats()` clears `linkParams`, and unlike ATT MTU beside it — restored synchronously in `beginSession()` from `maximumWriteValueLength` — they can only return once a GATT read completes. The gap is real and honest; it just said nothing. They were the only values on that screen whose absence was silent, against #230's `Not reported` and #242's `Reading…` / `Not responding.`
- **`offersLinkParams` is a tri-state**, and that is the substance of the fix rather than an implementation detail. *Not yet known* and *never going to answer* must not look alike: only `true` shows `Reading…`, while `false` — every simulator and every board older than #224 — keeps the rows **absent**, because those are not reading and are never going to answer.
- **`nil` is absent too**, which a first build got wrong: it admitted `nil` on the reasoning that waiting is what is happening, so every simulator connect inserted three `Reading…` rows and removed them one round trip later — this issue's own complaint inverted. The word was not even accurate there; no read had been issued yet. Gating on a positive answer also matches the requirement literally: *while connected with the characteristic discovered but no value yet*. Found in review.
- **`offersLinkParams` is latched across `resetLinkStats()`**, not cleared by it — it describes the peripheral's GATT database, which is stable across a reconnect, and latching is what keeps the rows on screen through the reset rather than flashing out and back. Same rule that latches `displayName` and `identity` against a callback carrying nothing. The trade is a stale answer if a board is reflashed across #224 within one app run; it costs one round trip, and a stale `false` hides rows rather than showing wrong ones.
- **`Not responding` is measured from the read, not from the frame stream.** A first build inferred it from `linkStatus` being `.stalled`, which describes the *motion notify stream* and is independent of whether this read came back: an ATT error on the handle, or a payload that is not exactly 8 bytes — one firmware revision away, since `LinkParams.init?` does not tolerate trailing bytes the way `SophonIdentity` does — leaves frames streaming at 50 Hz while the rows say `Reading…` forever, which is the exact case the state was added for. Now measured from when the read was issued. ATT read errors are logged rather than silently discarded.
- **On disconnect the rows stay in place and read `Available while connected`** — the same words the transmit counters use for the same state, so two sections of one screen do not invent separate vocabulary for "there is no link". Nothing clears `linkParams` on disconnect (deliberately — that persistence is what keeps the rows from vanishing across `beginSession()`'s reset), so an interim build suppressed the stale values by hiding the rows. Reported from use: hiding them is the wrong half to fix, because a row that disappears cannot explain itself, which is this issue's own argument. The values are suppressed by the display state instead.
- **`Not responding` when the read never comes back**, applying #242's rule here: *a read that never returns is an answer, and presenting it as an ongoing wait is the same omission as a green dot with no data behind it.* Without it a board that advertises the characteristic and then goes quiet would say `Reading…` forever.
- **Keeping the previous connection's values through the gap was rejected**, not overlooked. iOS revises the granted interval on its own schedule — the whole reason #224 exists — so showing the last figure would present a stale number as current, which is the failure #237 was filed against.
- **Row structure carries the constraint #237 uncovered.** The absent gate is a plain observable check outside any `TimelineView`, and each of the three rows carries its own timeline, because a `TimelineView` is always exactly one List row: a branch rendering nothing inside one leaves a blank cell, and three rows inside one would stack into a single cell with no separators. A clock is needed at all because an unanswered read changes nothing observable.
- **The absence is explained once, in the section footer**, rather than left silent: *no link-parameters characteristic to read them from — every simulated Sophon, since Core Bluetooth exposes no API for connection parameters on either side, and every board older than #224.* The issue permits absence *or* saying so once; with a board showing three rows and a simulator showing none, silence was the wrong half of that choice. A footnote is what the rest of that section already uses to explain an absence, and three rows repeating a sentinel would be louder than the fact deserves.
- **RSSI in the detail view is now unconditional**, reading `Not reported` when never observed — the third requirement, and the same argument #230 made. The device list row still omits it: that row is an `HStack`, not a List cell, so it carries no blank-row hazard, and a trailing `Not reported` in a compact row is noise.
- `TEST-PLAN.md` gains **§4, 18 cases**, including 4.6 (a simulator must stay absent, not sit at `Reading…`) and 4.9 (the `Not responding` path, flagged as hard to time — record a miss rather than a pass).

### feat: measure RSSI while connected (#237)

- `readRSSI()` was never called anywhere in the app. `device.rssi` was written in exactly one place — `didDiscover` — so it only ever updated from **advertisements**. A Sophon board is `CONFIG_BT_MAX_CONN=1` and stops advertising the moment it is taken, so for the whole of a connected session the row held the last value captured *before* the link came up. It read as a rock-steady signal; it was a number that had stopped being measured.
- **Polled from `SophonHub` at 1 Hz, not from a view's `.task`** — the same lesson as #235's sweep, for a sharper reason. The issue asked for polling to stop "when nothing is on screen, as the stats poll already does", but that rule does not carry across: RSSI is displayed in the **device list** as well as the detail view. A first build that applied it literally left a connected board's list row ageing forever with nothing that could ever clear the suffix — this issue's own complaint moved one screen over. Polling is now bounded by the *role* (between `resume()` and `suspend()`) rather than by a view, decided in `applyRadioState()`, which the file already names as the single decision point for what should be running. The stats poll stays view-gated because it is a genuine ATT round trip whose results only the detail view shows.
- Results go through the same `ingestRSSI` path as advertised samples — both are the central's own receive strength for the same radio, so one smoothing path is correct and the model deliberately cannot tell the two sources apart.
- **The smoothing window is now bounded by age as well as sample count.** The two sources arrive three orders of magnitude apart — tens per second from advertisements, one per second while connected — so the fixed 8-sample window spanned 0.3 s in one case and **8 s** in the other. Eight seconds cannot follow a tablet being carried away from a board, which is the one check the issue exists to make possible.
- **That age bound also settles what happens across a disconnect, with no disconnect hook.** Samples describing the old link expire on their own, so a reconnect — possibly from somewhere else entirely — starts its mean from its own first reading instead of blending into the previous one.
- **A remembered reading no longer looks like a measured one.** Once a value stops being refreshed the row carries its age (`-52 dBm · 7s ago`) and dims. This matters most in the device list, where a connected board's reading is as old as the last visit to its detail view — the same argument that produced `Not reported` in #230 and `Released` in #233.
- **The cadence is justified by usefulness, not thrift.** `Read_RSSI` is a local controller command (Core v6.0 Vol 4 Part E §7.5.4), not an ATT round trip, so it buys no air traffic and spends no connection-event budget. That claim is load-bearing, so `TEST-PLAN.md` §3e measures it against a 100 ms stress build rather than asserting it — a null result at the shipped 2 s cadence would prove nothing.
- **A connected reading is credited to the last packet the app can prove arrived**, not to the moment the reply landed — caught by `swift-reviewer` before this shipped. HCI `Read_RSSI` returns the strength of the controller's *last received packet* and cannot say how long ago that was, while `peripheral.state` stays `.connected` for the whole supervision window after a board dies. Stamping the reply time would have left the row looking freshly measured for minutes on a dead link, while the State row two lines above read `Stalled` — this issue's own defect rebuilt in a new place, with *frozen and looks live* swapped for *refreshed by asking and looks live*. Readings describing a packet older than the window are dropped, so the age climbs and the two rows now go stale together at the same 5 s from the same evidence.
- Label and staleness come from **one function returning both**, because two functions sharing an invariant are only as safe as the next edit, and the state they can disagree in is an ageless, confident-looking number in stale styling — precisely what this issue exists to prevent.
- The detail row's nil check sits **outside** its `TimelineView`: a `TimelineView` is always one row, so a nil reading inside one renders an `EmptyView` in a real List cell — a blank line with separators. The age also coarsens with scale (`52s` → `10m` → `over an hour`) rather than printing `3612s ago`.
- `TEST-PLAN.md` gains **§3, 26 cases**, including the acceptance test that was impossible before — walk the tablet away and watch the value follow — and 3.7, which is what fails if the crediting rule above is not working. 3.7 has to run on an **iOS-to-iOS link**: a board's supervision timeout is 420 ms (#224), so a dead board leaves `.connected` before the 5 s staleness threshold can be crossed, and the case that looks decisive on a board gives the same result on fixed and unfixed code.
...
- **Corrected a claim in #214 that hardware does not support.** The board-restart detector was justified by "a quick power cycle can stay inside the supervision timeout, so iOS never reports a disconnect — this is the only reliable signal". Measured across repeated board resets, `Sophon restarts` stays at **0**: the SoC loses its connection state, stops servicing connection events, the central's supervision timeout expires, and the board returns through an ordinary reconnect. A peripheral cannot hold a link it has forgotten. The branch is kept because it is cheap and correct when it fires, and it *is* reachable — by a peripheral that restarts its application state on a live link, which today means the simulator's Reboot button and nothing else. For real hardware the reliable signal is the reconnect itself.
- Renamed **"App interruptions"** to **"Viewer not listening"**. The old label was unambiguous only while every peripheral was a board — there was one app in the system and it was this one. Since the simulator, the thing at the other end can be an app too, so on a screen describing a remote Sophon the label no longer said *whose* interruption it was.
- Renamed the counter to **"Restarts without disconnect"** (`restartsWithoutDisconnect`). It was "Sophon restarts", which promised to count restarts of the Sophon and then read 0 however many times the board was reset — because a board reset drops the link and returns as a reconnect, never reaching the detector. The only thing that trips it is a peripheral restarting its *application* state while holding the link open, which today means the simulator's Reboot button. A counter whose name overstates what it measures is the same defect as a green dot with no data behind it.
- It is now always shown, including at zero, matching the reasoning already applied to the interruption count one row below: a field that appears only once something has gone wrong cannot be used to confirm that nothing has. Its absence was being read as "not measured" rather than "none".
...
- There is one board, so every app change was gated on it being flashed and to hand, and #211 — multi-device batching and time sync — could not be exercised at all. The app now has a **simulator mode**: an iPhone advertises the Sophon Motion Service under an ordinary `Sophon-XXXX` name and streams its own CoreMotion accelerometer and gyroscope in the frozen 18-byte frame at ~52 Hz, so an iPad running viewer mode sees it as a board. Indistinguishable on purpose — the viewer contains no special case for it.
- One app with a mode switch rather than a second target, so the new **encoder sits directly beneath the decoder** in `SophonProtocol.swift`. Neither `MotionFrame` nor `TxStats` could previously be constructed in code at all: each had only a failable `init?(Data)`, which suppresses the memberwise init.
- The wire format is checked at launch against **byte vectors transcribed from `PROTOCOL.md`**, not against our own decoder. A round trip is self-consistent by construction — swap the endianness of both halves and it still passes while the board and the phone disagree completely. This is not theoretical: there is no test target, and the app now contains both halves of a contract whose third implementer is firmware.
- `Double` → `Int16` conversion **saturates**, mirroring `clamp16()` in `imu.c`. `Int16(200_000.0)` traps, and an iPhone gyro clears the frame's 327.67 dps ceiling on a flick of the wrist, so this runs in ordinary use. The self-check immediately caught a real bug in it: guarding on `isFinite` sent infinity to zero, quietly reporting a violent off-scale reading as *stationary*.
- **Streaming survives backgrounding**, which needed more than the obvious. `bluetooth-peripheral` keeps a peripheral *wakeable* for incoming events, but this one is push-driven — nothing asks it for a frame — so once suspended nothing wakes it and the stream stops. A `location` session holds the process scheduled; no location is read or stored. `pausesLocationUpdatesAutomatically = false` is the line that decides whether it works, since iOS otherwise pauses updates once it judges the device stationary, which a phone simulating a sensor on a desk certainly is.
- **Entering simulator mode suspends the central.** A real board is `CONFIG_BT_MAX_CONN=1`, so a phone that kept scanning while impersonating one would take the real board's only connection slot. Suspension had to defeat an existing unconditional re-arm in `didDisconnectPeripheral`, and cancels *every* known peripheral rather than only connected ones — an outstanding iOS `connect()` never times out.
- Bench controls make the viewer's attribution testable for the first time: **drop 0–20% of frames** reproduces the firmware's out-of-buffers path, which iOS's generous transmit queue never produces on its own, and **reboot** resets `seq` and uptime without dropping the link — the only way to reach the board-restart detector, since switching modes disconnects and clears it first.
- `scripts/deploy-device.sh` gains an optional device selector and `--all`; with two usable devices and no selector it now fails with a paste-ready command per candidate rather than silently taking whichever enumerated first. Two attached devices is the normal case for this feature.
- The script now reads `devicectl`'s **JSON** rather than scraping its table — devicectl's own help calls JSON the only supported interface for scripts, and the table is genuinely hostile to parse: device names contain spaces so columns shift, and `available (paired)` is two tokens where `connected` is one. It reports each device's **tunnel state** up front, and **reports rather than obeys** it: a `disconnected` tunnel is not a reason to skip a device, since devicectl usually establishes one on demand — measured, all three devices installed cleanly from `disconnected`. Because that is not reliable either (the same device failed minutes earlier with `Connection reset by peer`), a failed install is retried once, the first attempt often being what wakes the tunnel. An earlier revision filtered on `connected` only and silently skipped every device that would have worked.
- **Verified on hardware**, iPhone 17e advertising to an iPad and to a Mac acting as a third central simultaneously: the advertised `Sophon-XXXX` name survives in the advertisement, and the stream held **50.0 Hz for ~8 minutes with the phone locked and stationary** — every 10 s bucket between 498 and 503 frames, 20 sequence gaps in ~22800 frames, no disconnects. That last test is the one that justifies `pausesLocationUpdatesAutomatically = false`: iOS otherwise pauses location updates once it judges the device stationary, minutes in, and the symptom would have looked like CoreMotion failing.
- Measured rate is **50.0 Hz, not the requested 52** — CoreMotion quantises the 19.23 ms interval up to 20 ms. The board runs ~54.3, so the two sit **8% apart while both claim 52**, in opposite directions. More evidence that `t_ms` is the only timebase worth trusting.
- **Core Bluetooth state restoration was tried and removed.** With a restore identifier, killing the app got it relaunched in the background to restore the session — service back, advertising resumed, central reconnected, seen as the viewer flicking to Disconnected and straight back to Connected. But a restoration launch has no UI and sampling is started from the view, so CoreMotion never restarted: the result was a zombie peripheral, advertising and subscribed and permanently silent. The viewer was not wrong to say Connected — the link was real, only the data was missing — and that is a worse failure than being absent, because absence is unambiguous. A killed simulator now stays dead, which is also how a powered-off board behaves.
- Documented the divergences honestly in `PROTOCOL.md`: MTU ~185 rather than 23, several centrals rather than one, and **axes in Apple's frame, so signs and axis order will not match a real board** — which #210's fusion will have to check against hardware.
